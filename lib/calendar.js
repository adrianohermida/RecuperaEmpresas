...
// One-time startup check — logs clearly when integration is disabled
let _credWarned = false;
function _warnMissingCreds() {
  if (_credWarned) return;
  _credWarned = true;
  console.warn('[GCAL] ⚠️  Google Calendar integration is DISABLED.');
  console.warn('[GCAL]     OAuth2 needs: GOOGLE_CLIENT_ID + GOOGLE_CLIENT_SECRET + GOOGLE_OAUTH_REFRESH_TOKEN');
  console.warn('[GCAL]     Service account needs: GOOGLE_CLIENT_EMAIL + GOOGLE_PRIVATE_KEY');
  console.warn('[GCAL]     Set at least one set of credentials to enable calendar sync.');
}
...
async function _gcAccessToken() {
  const token = (await _oauthAccessToken()) || (await _serviceAccountToken());
  if (!token) _warnMissingCreds();
  return token;
}
 ...
 */
/**
 * Parse a local date+time string in America/Sao_Paulo (UTC-3, fixed — Brazil
 * abolished DST in April 2019) into a UTC Date object.
 * We avoid `new Date("YYYY-MM-DDTHH:MM:SS")` which is parsed as local server
 * time (UTC on most cloud servers), producing a 3-hour shift.
 */
function _spDate(dateStr, timeStr) {
  // America/Sao_Paulo = UTC-3 year-round since 2019
...
function computeFreeWindows(busyIntervals, dateStr, slotMinutes = 60, workHours = { start: '08:00', end: '18:00' }) {
  const dayStart = _spDate(dateStr, workHours.start);
  const dayEnd   = _spDate(dateStr, workHours.end);
