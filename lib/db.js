...
const { auditLog } = require('./logging');
...
async function saveChapterContent(userId, chapterId, content, editorId, attachments = []) {
  ...
  // BP-BE-04: Audit log for chapter content save
  auditLog({
    actorId: editorId,
    actorRole: 'consultor',
    entityType: 're_plan_chapters',
    entityId: `${userId}:${chapterId}`,
    action: 'edit',
    notes: 'Capítulo editado via backend principal.'
  });
}
...
async function publishChapterForApproval(userId, chapterId, consultorId) {
  ...
  auditLog({
    ...
    entityId: `${userId}:${chapterId}`,
    action: 'publish',
    notes: 'Capítulo publicado para aprovação do cliente.'
  });
}
...
async function approveChapter(userId, chapterId, clientId) {
  ...
  // BP-BE-04: Audit log for chapter approval
  auditLog({
    actorId: clientId,
    actorRole: 'cliente',
    entityType: 're_plan_chapters',
    entityId: `${userId}:${chapterId}`,
    action: 'approve',
    notes: 'Capítulo aprovado pelo cliente.'
  });
}
...
async function requestChapterRevision(userId, chapterId, clientId, revisionReason) {
  ...
  // BP-BE-04: Audit log for revision request
  auditLog({
    actorId: clientId,
    actorRole: 'cliente',
    entityType: 're_plan_chapters',
    ...
    notes: 'Revisão solicitada pelo cliente.'
  });
}
