 ...
 */
async function createConversation({ client_id, consultant_id = null, subject = null }) {
  const { data, error } = await sb
    .from('re_chat_conversations')
    .insert({ client_id, consultant_id, subject })
    .select()
    .single();
...
async function listConversationsForConsultant(consultant_id, status = null) {
  let query = sb
    ...
    .select(`
      *,
      re_users!re_chat_conversations_client_id_fkey(name, company)
    `)
