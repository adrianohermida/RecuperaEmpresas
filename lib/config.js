...
const GOOGLE_RECAPTCHA_MIN_SCORE = Number(process.env.GOOGLE_RECAPTCHA_MIN_SCORE || process.env.RECAPTCHA_MIN_SCORE || '0.5');

const ADMIN_EMAILS = (process.env.ADMIN_EMAILS || 'contato@recuperaempresas.com.br,camilaghmaia@gmail.com,adrianohermida@gmail.com')
                       .split(',').map(e => e.trim().toLowerCase());

// ─── Google Calendar — OAuth2 user credentials (Camila's account) ────────────
// Primary: OAuth2 refresh_token flow (uses GOOGLE_CLIENT_ID + SECRET + REFRESH_TOKEN)
// Fallback: service account (GOOGLE_CLIENT_EMAIL + PRIVATE_KEY) if OAuth2 not configured
...
const GOOGLE_ACCESS_TOKEN   = process.env.GOOGLE_ACCESS_TOKEN   || ''; // initial seed only
// Service account fallback
const GOOGLE_CLIENT_EMAIL   = process.env.GOOGLE_CLIENT_EMAIL   || '';
const GOOGLE_PRIVATE_KEY    = (process.env.GOOGLE_PRIVATE_KEY   || '').replace(/\\n/g, '\n');
const GOOGLE_CALENDAR_ID    = process.env.GOOGLE_CALENDAR_ID    || 'primary';
const GOOGLE_CALENDAR_TZ    = process.env.GOOGLE_CALENDAR_TZ    || 'America/Sao_Paulo';
// Webhook channel for push notifications (set to your public BASE_URL)
const GOOGLE_CALENDAR_WEBHOOK_SECRET = process.env.GOOGLE_CALENDAR_WEBHOOK_SECRET || '';
